...
sequenceDiagram
    ...
    participant Service as RustDesk Service (SYSTEM)
    ...
    alt New Version Available
        ...
        alt Windows Desktop (Service Installed)
            Client->>Service: Delegate update via IPC (UpdateMe)
            Service->>OS: Execute installer silently as SYSTEM (no UAC)
            OS->>Client: Terminate client process and overwrite files
        else Windows Desktop (Portable / No Service)
